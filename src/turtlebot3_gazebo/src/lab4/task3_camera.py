...
class Map():
    ...
    def __open_map(self,map_name):
        ...
        map_name = 'src/turtlebot3_gazebo/maps/' + map_name
        im = cv2.imread(map_name, cv2.IMREAD_GRAYSCALE)
        if im is None:
            raise FileNotFoundError(f"Map image '{map_name}.pgm' not found.")
        im_width,im_height = im.shape
        print(im_width,im_height)
        size_height = 200
        size_width = int(size_height*im_width/im_height)
        im = cv2.resize(im, (size_width,size_height), interpolation=cv2.INTER_AREA)
        xmin = map_df.origin[0][0]
        xmax = map_df.origin[0][0] + im.shape[0] * map_df.resolution[0]
        ymin = map_df.origin[0][1]
        ymax = map_df.origin[0][1] + im.shape[1] * map_df.resolution[0]
        xmin_old = map_df.origin[0][0]
        xmax_old = map_df.origin[0][0] + im_height * map_df.resolution[0]
        ymin_old = map_df.origin[0][1]
        ymax_old = map_df.origin[0][1] + im_width * map_df.resolution[0]
        print(im.shape[1])
...
class Task3(Node):
    ...
    def image_callback(self,msg):
        try:
            ...
            self.objects = self.process_image(image)            
            msg = Twist()
    ...
    def __real_world_to_grid(self, data):        
        ...
        xscale = self.mp.map.map_im.shape[0]/(self.mp.map.old_limits[1]-self.mp.map.old_limits[0])
        yscale = self.mp.map.map_im.shape[1]/(self.mp.map.old_limits[3]-self.mp.map.old_limits[2])
